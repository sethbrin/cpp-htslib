...
typedef struct multithread_mplp_conf_t {
  htsFile *bcf_fp;
  bcf_hdr_t *bcf_hdr;
  mplp_conf_t *conf;
  FILE *pileup_fp;
  ...
  mplp_aux_t** data;
  mplp_ref_t* mp_ref;
  ncic::easehts::mpileup::PileupBuffer* pileup_buffers;
  int block_size;
  int max_read_length;

  multithread_mplp_conf_t()
      : bcf_fp(NULL),
        bcf_hdr(NULL),
        conf(NULL),
        pileup_fp(NULL),
...
static void initHeader(multithread_mplp_conf_t* multithread_conf, int n, char **fn) {

  mplp_ref_t* mp_ref = multithread_conf->mp_ref;
  mplp_conf_t*& conf = multithread_conf->conf;
  bam_sample_t*& sm = multithread_conf->conf;
  mplp_aux_t**& data = multithread_conf->data;
  bam_sample_t* sm = bam_smpl_init();
  multithread_conf->sm = sm;
  ...
  if (conf->flag & MPLP_BCF)
  {
    ...
    multithread_conf->bcf_fp = bcf_open(conf->output_fname? conf->output_fname : "-", mode);
    if (multithread_conf->bcf_fp == NULL) {
      fprintf(stderr, "[%s] failed to write to %s: %s\n", __func__, conf->output_fname? conf->output_fname : "standard output", strerror(errno));
      exit(EXIT_FAILURE);
    }

    // BCF header creation
    bcf_hdr_t*& bcf_hdr = multithread_conf->bcf_hdr;
    ...
    bcf_hdr_write(multithread_conf->bcf_fp, bcf_hdr);
    // End of BCF header creation

  }
  else {
    multithread_conf->pileup_fp = conf->output_fname? fopen(conf->output_fname, "w") : stdout;

    if (multithread_conf->pileup_fp == NULL) {
...
static int mpileup(mplp_conf_t *conf, int n, char **fn) {
  multithread_mplp_conf_t multithread_conf;
  multithread_conf.conf = conf;
  mplp_ref_t mp_ref = MPLP_REF_INIT;
  multithread_conf.mp_ref = &mp_ref;
  multithread_conf.block_size = 1000000;
  multithread_conf.max_read_length = 150;
  initHeader(&multithread_conf, n, fn);

  ncic::easehts::mpileup::PileupBuffer pileup_buffers;
  multithread_conf.pileup_buffers = &pileup_buffers;

  std::vector<std::thread> workers;
  for (size_t i = 0; i = 10; i++) {
    workers.emplace_back(worker, std::cref(multithread_conf), n, fn);
  }

  std::thread producer([&multithread_conf]() {
    // just test the first sam/bam file to get region
    mplp_aux_t* ma = multithread_conf.data[0];
    int count = 0;
    int begin = -1;
    int end;
    bam1_t* b = bam_init1();
    do {
      int ret = ma->iter? sam_itr_next(ma->fp, ma->iter, b) : sam_read1(ma->fp, ma->h, b);
      if (ret < 0) break;
      count ++;
      if (count >= multithread_conf.block_size) {
        if (begin == -1 && conf->reg) {
          begin = ma->iter->beg - multithread_conf.max_read_length;
        }
        end = b->core.pos;
        pileup_buffers.Add(begin, end);
  ...
  std::thread writer([&pileup_buffers] {
    std::shared_ptr<ncic::easehts::mpileup::PackData> pd;
    while (true) {
      pd = pileup_buffers.PopFront();
      if (pd.get() == NULL) {
        break;
      }
  ...
  if (multithread_conf.bcf_fp)
  {
    hts_close(multithread_conf.bcf_fp);
    bcf_hdr_destroy(multithread_conf.bcf_hdr);

  }
  if (multithread_conf.pileup_fp && conf->output_fname) fclose(multithread_conf.pileup_fp);
  bam_smpl_destroy(multithread_conf.sm);
 ...
 * @param fn filenames
 */
static int worker(const multithread_mplp_conf_t& multithread_conf, int n, char **fn)
{
  ...
  htsFile *bcf_fp = multithread_conf.bcf_fp;
  bcf_hdr_t *bcf_hdr = multithread_conf.bcf_hdr;
  mplp_conf_t *conf = multithread_conf.conf;
  bam_sample_t*& sm = multithread_conf.sm;
  ...
  for (int i = 0; i < n; ++i) {
    ...
    data[i]->conf = conf;
    data[i]->ref = multithread_conf.mp_ref;
  ...
  while (true) {
    std::list<std::shared_ptr<ncic::easehts::mpileup::PackData>>::iterator pack_data_iter =
        multithread_conf.pileup_buffers->GetProcessPackData();
    std::shared_ptr<ncic::easehts::mpileup::PackData> pack_data = *pack_data_iter;
    ...
    for (int i = 0; i < n; ++i) {
      ...
      bam_hdr_t* h_tmp = sam_hdr_read(data[i]->fp);
      if ( (data[i]->iter=sam_itr_queryi(idx, h_tmp, pack_data->begin, pack_data->end)) == 0) {
        fprintf(stderr, "[E::%s] fail to parse region '%s' with %s\n", __func__, conf->reg, fn[i]);
        exit(EXIT_FAILURE);
      }
